// Qualiopi screen — indicator dashboard + checklist + missing pieces
const QualiopiScreen = () => {
          ...
          <div className="kpi" key={i}>
            ...
            <div className="value" style={{
              color: s.tone === 'ok' ? 'var(--ok)' :
                     ...
                     s.tone === 'err' ? 'var(--err)' :
                     s.tone === 'accent' ? 'var(--accent)' : 'var(--ink)'
            }}>{s.v}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 18 }}>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          ...
          <table className="t">
            ...
            <tbody>
              {QUALIOPI.map(q => {
                const map = { ok: 'ok', warn: 'warn', err: 'err', review: 'warn' };
                const labels = { ok: 'Validé', warn: 'À compléter', err: 'Bloqué', review: 'En relecture' };
                return (
                  <tr key={q.num} onClick={() => setSelected(q.num)} style={{ background: selected === q.num ? 'var(--surface-2)' : 'transparent' }}>
                    ...
                    <td><Badge kind={map[q.status]} dot>{labels[q.status]}</Badge></td>
        ...
        <div className="card">
          <div className="card-hd">
            ...
            <Badge kind={ind.status === 'ok' ? 'ok' : ind.status === 'err' ? 'err' : 'warn'} dot>
              {ind.status === 'ok' ? 'Validé' : ind.status === 'err' ? 'Bloqué' : 'À compléter'}
            </Badge>
          </div>
          ...
          <div className="card-bd" style={{ paddingTop: 0 }}>
            ...
            <div style={{ marginTop: 14 }}>
              ...
              <div className="col" style={{ gap: 6 }}>
                {[
                  ...
                  { name: 'Échantillon de 5 livrets pédagogiques', ok: true, type: 'zip' },
                  { name: 'Attestation présence formateurs', ok: true, type: 'pdf' },
                  { name: 'Synthèse statistique d\'assiduité Q1', ok: true, type: 'xls' },
                  ...
                  <div key={i} style={{
                    ...
                    background: p.ok ? 'transparent' : 'var(--warn-soft)',
                    border: `1px solid ${p.ok ? 'var(--line)' : '#f0c98c'}`,
            ...
            <div style={{
              ...
              border: '1px solid var(--accent-soft)', borderRadius: 12,
            }}>
              <div className="row" style={{ gap: 8, marginBottom: 6 }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.5 }}>
                Les pièces <em>« Plan de remédiation »</em> et <em>« Échantillon évaluations »</em> existent peut-être dans le dossier <strong>Pédagogie / Suivi 2025</strong>. Voulez-vous que je les recherche et les rattache automatiquement ?
              </div>
...
window.QualiopiScreen = QualiopiScreen;
