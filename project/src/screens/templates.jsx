...
const TemplatesScreen = ({ onShowToast }) => {
  ...
  const useTemplate = (tpl) => {
    onShowToast({ message: `Nouveau document créé depuis "${tpl.name}"`, icon: 'file' });
    setPreviewTpl(null);
  };
    ...
    <div className="page" style={{ paddingTop: 22 }}>
      ...
      <div className="row" style={{ gap: 10, marginBottom: 18, flexWrap: 'wrap' }}>
        <div className="search" style={{ width: 300 }}>
          ...
          <input placeholder="Rechercher un modèle…" value={searchQ} onChange={e => setSearchQ(e.target.value)} style={{ flex: 1 }} />
        </div>
      ...
      {view === 'grid' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
          {shown.map(tpl => (
            <div key={tpl.id} className="card" style={{ padding: 18, cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 12, transition: 'box-shadow .15s' }}
        ...
        <div className="modal-bg" onClick={() => setShowNewModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div><label className="label">Fichier modèle (Word, PDF, Excel)</label>
            ...
            <div style={{ padding: '14px 24px', borderTop: '1px solid var(--line)', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button className="btn primary" onClick={() => { setShowNewModal(false); onShowToast({ message: 'Modèle créé et ajouté à la bibliothèque', icon: 'check' }); }}>
