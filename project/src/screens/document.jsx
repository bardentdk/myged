// Document detail — the centerpiece. Shows the lock state when Sylvie is editing,
// and Anne's (ME) full set of actions.
const DocumentScreen = ({ docId, onNavigate, onShowToast, scenario, onScenarioChange }) => {
  const doc = DOCS.find(d => d.id === docId) || DOCS[0];
  const [tab, setTab] = useState('preview'); // preview | versions | activity | comments | permissions
  ...
  const handleRequestUnlock = () => { setRequestSent(true); onShowToast({ message: `Demande envoyée à ${lockOwner.name.split(' ')[0]}`, icon: 'send' }); };
  const handleNotify = () => { setNotifyOn(true); onShowToast({ message: 'Vous serez notifié·e dès que le document sera libéré', icon: 'bell' }); };
  const handleDownloadCopy = () => { setShowCopyConfirm(true); setTimeout(() => setShowCopyConfirm(false), 2400); onShowToast({ message: `Copie téléchargée — ${doc.name}`, icon: 'download' }); };
  const handleForceUnlock = () => { setShowForceModal(false); onScenarioChange('released'); onShowToast({ message: `Document déverrouillé. Sylvie a été notifiée.`, icon: 'unlock' }); };
  const handleRelease = () => { onScenarioChange('released'); onShowToast({ message: `Sylvie vient de libérer le document — c'est à vous.`, icon: 'unlock' }); };

  return (
    <div className="page" style={{ paddingTop: 18, maxWidth: 'none', padding: '18px 24px 60px' }}>
      {/* Back link */}
      <button onClick={() => onNavigate('explorer')} className="btn ghost sm" style={{ marginBottom: 12, color: 'var(--ink-3)' }}>
        ...
        <div className="row" style={{ gap: 6 }}>
          ...
          {isLocked && !lockedByMe ? (
            <button className="btn sm" disabled style={{ opacity: .55, cursor: 'not-allowed' }}>
              <Icon name="lock" size={13} /> Modifier
            </button>
          ) : (
      ...
      {isLocked && !lockedByMe && (
        <LockBanner
          owner={lockOwner}
          since={doc.lockedSince}
          requestSent={requestSent}
          notifyOn={notifyOn}
          onCopy={handleDownloadCopy}
          onNotify={handleNotify}
          onRequest={handleRequestUnlock}
          onForce={() => setShowForceModal(true)}
          onSimulateRelease={handleRelease}
        />
      )}
      {scenario === 'released' && (
        <ReleasedBanner onEdit={() => onShowToast({ message: 'Édition démarrée — document verrouillé par vous', icon: 'lock' })} />
      )}

      {/* 3-column layout: preview | tabs/info */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 18, marginTop: 14 }}>
        <div className="card" style={{ overflow: 'hidden' }}>
          ...
          <div style={{ minHeight: 540 }}>
            ...
            {tab === 'activity' && <ActivityList doc={doc} />}
...
const LockBanner = ({ owner, since, requestSent, notifyOn, onCopy, onNotify, onRequest, onForce, onSimulateRelease }) => (
  <div className="card" style={{
    borderColor: '#f0c98c', background: 'linear-gradient(180deg, #fdf6ea, #fcf2e2)',
    boxShadow: '0 1px 0 rgba(180,83,9,.08), 0 4px 12px rgba(180,83,9,.06)'
  }}>
    <div style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 16 }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: 12.5, color: '#92400e' }}>
          Verrouillé depuis <span className="mono">{since}</span> · vous ne pouvez pas écraser sa version, mais voici ce que vous pouvez faire.
        </div>
      </div>
      <div className="row" style={{ gap: 8 }}>
        <button className="btn sm" onClick={onCopy}>
          <Icon name="download" size={13} /> Télécharger une copie
        </button>
        ...
        <button className="btn sm danger" onClick={onForce} title="Réservé aux admins">
          <Icon name="unlock" size={13} /> Forcer
        </button>
      </div>
    </div>
    {/* Simulator helper line */}
    <div style={{ padding: '8px 18px', borderTop: '1px dashed rgba(180,83,9,.25)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12 }}>
      ...
      <button className="btn sm ghost" onClick={onSimulateRelease} style={{ color: '#92400e' }}>
        <Icon name="play" size={11} /> Simuler la libération
      </button>
    </div>
  </div>
);
...
const ReleasedBanner = ({ onEdit }) => (
  <div className="card" style={{
    borderColor: '#9fd3b5', background: 'linear-gradient(180deg, #f1faf3, #e7f6ec)',
  }}>
    <div style={{ padding: '12px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: 12.5, color: '#15803d' }}>
          Sylvie a publié la version <span className="mono">2.3</span> il y a 12 secondes. Vous pouvez maintenant le modifier sans conflit.
        </div>
      </div>
      <button className="btn primary sm" onClick={onEdit}>
        <Icon name="edit" size={13} /> Modifier maintenant
      </button>
    </div>
  </div>
);
  ...
  <div className="modal-bg" onClick={onCancel}>
    <div className="modal" onClick={e => e.stopPropagation()}>
      ...
      <div style={{ padding: '18px 24px', fontSize: 13.5, lineHeight: 1.6, color: 'var(--ink-2)' }}>
        <p style={{ marginTop: 0 }}>
          <strong>{owner.name}</strong> est en train de modifier ce document depuis 23 minutes.
          Forcer le déverrouillage peut <strong>provoquer la perte de ses modifications non sauvegardées</strong>.
        </p>
        <div style={{ background: 'var(--surface-2)', padding: 12, borderRadius: 8, border: '1px solid var(--line)', fontSize: 12.5 }}>
          <div className="row" style={{ gap: 8 }}>
            ...
            <span>{owner.name.split(' ')[0]} sera notifiée immédiatement et pourra téléverser sa version sous forme de copie de secours.</span>
          </div>
        </div>
...
const PreviewPane = ({ doc, isLocked, lockOwner }) => (
  <div style={{ padding: 28, background: 'var(--surface-2)', minHeight: 540, position: 'relative' }}>
    {/* Fake document */}
    <div style={{
      background: '#fff', maxWidth: 620, margin: '0 auto', padding: '48px 56px',
      boxShadow: '0 4px 24px rgba(0,0,0,.06)', borderRadius: 4,
      fontFamily: 'Georgia, serif', color: '#1a1a1a', position: 'relative', fontSize: 12.5, lineHeight: 1.55,
    }}>
      {isLocked && (
        <div style={{ position: 'absolute', top: 14, right: 14, display: 'flex', alignItems: 'center', gap: 6,
          background: '#fcf2e2', color: '#92400e', padding: '4px 10px', borderRadius: 999, fontSize: 11, fontFamily: 'Geist, sans-serif',
      ...
      <div style={{ textAlign: 'center', marginBottom: 24, paddingBottom: 16, borderBottom: '1px solid #e5e5e5' }}>
        ...
        <div style={{ fontSize: 11, color: '#666', marginTop: 6 }}>N° d'enregistrement : 974-26-00471 · établie en application des articles L.6353-1 et suivants du Code du travail</div>
      </div>
      ...
      <p>d'une part,</p>
      <p>et l'employeur <strong>SARL Océan Indien</strong>, SIRET 482 309 174 00021, représenté par Mme Mélanie Grondin, Directrice Générale, ci-après désigné « l'Employeur »,</p>
      <p>d'autre part,</p>
      <p style={{ marginTop: 18 }}><strong>Article 1 — Objet de la convention</strong></p>
      <p>La présente convention a pour objet la formation professionnelle continue de l'apprenant Jean Payet, ci-après désigné « le Bénéficiaire », dans le cadre de la préparation au Titre Professionnel Secrétaire Assistant·e de niveau 4.</p>
      <p><strong>Article 2 — Nature et durée de la formation</strong></p>
      <p>L'action de formation entre dans la catégorie des actions définies à l'article L.6313-1 du Code du travail. Elle se déroule sur <strong>805 heures</strong>, du 12 janvier 2026 au 28 août 2026, sur le site de Saint-Denis…</p>
    ...
    {VERSIONS.map((v, i) => {
      ...
      return (
        <div key={v.v} style={{
          ...
          borderBottom: i < VERSIONS.length - 1 ? '1px solid var(--line)' : 0,
          background: v.current ? 'var(--accent-soft)' : 'transparent',
          position: 'relative',
        }}>
          ...
          <div style={{ flex: 1 }}>
            <div className="row" style={{ gap: 8, marginBottom: 4 }}>
              ...
              {v.locked && <Badge kind="warn" dot>En édition</Badge>}
            ...
            <div className="row" style={{ gap: 6, fontSize: 12.5, color: 'var(--ink-3)', marginBottom: 6 }}>
              ...
              <span><strong style={{ color: 'var(--ink-2)', fontWeight: 500 }}>{u.name}</strong> · {v.date}</span>
            </div>
            ...
            <div className="row" style={{ gap: 6, marginTop: 8 }}>
              ...
              {!v.current && <button className="btn sm ghost"><Icon name="refresh" size={12} /> Restaurer cette version</button>}
              {!v.current && <button className="btn sm ghost"><Icon name="layers" size={12} /> Comparer à v{VERSIONS[0].v}</button>}
            </div>
          </div>
        </div>
      );
...
const ActivityList = ({ doc }) => {
  const items = [
    { who: 'sylvie',  verb: 'a verrouillé', when: 'aujourd\'hui · 14:32', type: 'lock' },
    { who: 'sylvie',  verb: 'a ouvert',     when: 'aujourd\'hui · 14:30', type: 'eye' },
    ...
    { who: 'anne',    verb: 'a ajouté un commentaire', when: '03 mars · 16:50', type: 'msg' },
    { who: 'sylvie',  verb: 'a remplacé par v2.2', when: '12 mars · 09:14', type: 'upload' },
        ...
        return (
          <div key={i} style={{ display: 'flex', gap: 12, padding: '10px 0', borderBottom: i < items.length - 1 ? '1px solid var(--line)' : 0 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div className="micro mono" style={{ marginTop: 2 }}>{a.when} · 192.168.1.{42 + i}</div>
            </div>
            <Icon name={a.type} size={14} style={{ color: 'var(--ink-4)', marginTop: 6 }} />
          </div>
        );
      })}
    </div>
...
const CommentsPane = ({ comment, setComment }) => (
  <div style={{ padding: 22 }}>
    {[
      ...
      { who: 'jb',      text: "Pensez à vérifier les clauses de financement OPCO — Akto a mis à jour son barème en février.", when: 'il y a 1 j' },
    ].map((c, i) => {
      const u = USERS[c.who];
      return (
        <div key={i} style={{ display: 'flex', gap: 12, padding: '14px 0', borderBottom: '1px solid var(--line)' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div className="row" style={{ gap: 12, marginTop: 8, fontSize: 12, color: 'var(--ink-3)' }}>
              <span style={{ cursor: 'pointer' }}>Répondre</span>
              <span style={{ cursor: 'pointer' }}>Résoudre</span>
            </div>
          </div>
        </div>
      );
      ...
      <div style={{ flex: 1 }}>
        ...
        <div className="row" style={{ justifyContent: 'space-between', marginTop: 8 }}>
          <div className="row" style={{ gap: 4 }}>
            <button className="btn ghost sm" style={{ padding: 4 }}><Icon name="paperclip" size={13} /></button>
            <button className="btn ghost sm" style={{ padding: 4 }}><Icon name="user" size={13} /></button>
          </div>
...
const PermissionsTable = () => {
  const rows = [
    ...
    { who: 'thierry', role: 'Qualiopi',     perms: { r: 1, d: 1, w: 0, v: 0, s: 0, x: 0 } },
  ...
  return (
    <div style={{ padding: 22 }}>
        ...
        <thead>
          <tr>
            ...
            {Object.entries(labels).map(([k, v]) => (
              <th key={k} style={{ textAlign: 'center', padding: '8px 4px' }}>{v}</th>
            ))}
          </tr>
        </thead>
        <tbody>
      ...
      <button className="btn sm" style={{ marginTop: 14 }}><Icon name="plus" size={12} /> Ajouter un utilisateur</button>
    </div>
  );
};
...
const DetailsCard = ({ doc, isLocked, lockOwner }) => {
  const rows = [
    ...
    { label: 'Dossier',         v: <a href="#" style={{ color: 'var(--accent)', textDecoration: 'none' }}>{doc.folder.split(' / ').pop()}</a> },
    { label: 'Modifié',         v: <span>{doc.updatedAt}</span> },
    ...
    { label: 'Verrouillage',    v: isLocked ? (
      <span className="row" style={{ gap: 6 }}>
        <Avatar user={lockOwner} size="xs" />
        <span style={{ fontSize: 12.5 }}>{lockOwner.name.split(' ')[0]} · <span className="mono">14:32</span></span>
      </span>
  ...
  <div className="card">
    ...
    <div>
      {[
        ...
        { name: 'Convocation Jean Payet',       type: 'pdf', v: '1.0' },
      ].map((d, i) => (
...
Object.assign(window, { DocumentScreen });
