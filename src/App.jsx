...
import Explorer from './screens/Explorer.jsx';
import DocumentScreen from './screens/Document.jsx';
import VersionsScreen from './screens/Versions.jsx';
import QualiopiScreen from './screens/Qualiopi.jsx';
import AiPanel from './screens/AI.jsx';
...
const App = () => {
  ...
  const [lockScenario, setLockScenario] = useState('locked');
      ...
      <div className="toast-wrap">
        {toasts.map(toast => (
          <div key={toast.id} className="toast">
            {toast.icon && <Icon name={toast.icon} size={14} style={{ color: '#a3e5b8' }} />}
            <span>{toast.message}</span>
            <span className="toast-x" onClick={() => setToasts(ts => ts.filter(x => x.id !== toast.id))}>×</span>
          </div>
        ))}
      </div>
