  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.05fr', minHeight: '100vh', background: 'var(--bg)' }}>
      <motion.section
        ...
        style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '40px 60px' }}
      >
        <div className="row" style={{ gap: 10 }}>
          <span className="dot-mark" style={{ width: 11, height: 11 }} />
          <span style={{ fontWeight: 700, fontSize: 17, letterSpacing: '-0.02em' }}>Mar'my GED</span>
        </div>
        ...
        <div style={{ maxWidth: 380, width: '100%', margin: '0 auto' }}>
          {step === 'credentials' && (
            <form onSubmit={handleSubmit}>
              ...
              {error && (
                <div style={{ marginBottom: 14, padding: '10px 14px', background: 'var(--err-soft)', border: '1px solid var(--err)', borderRadius: 8, fontSize: 13, color: 'var(--err)' }}>
                  {error}
                </div>
              )}
              ...
              <button type="submit" className="btn primary lg" disabled={loading}
                style={{ width: '100%', justifyContent: 'center', marginTop: 22, padding: '11px 16px' }}>
                {loading ? 'Connexion…' : <><span>Continuer</span><Icon name="arrowR" size={14} /></>}
              </button>
              ...
              <button type="button" className="btn lg" style={{ width: '100%', justifyContent: 'center' }}>
                <svg width="14" height="14" viewBox="0 0 24 24">
                  <path fill="#4285F4" d="M22 12c0-.7-.06-1.4-.18-2H12v3.83h5.6c-.24 1.3-.97 2.4-2.07 3.13v2.6h3.35c1.96-1.8 3.12-4.46 3.12-7.56Z"/>
                  <path fill="#34A853" d="M12 22c2.8 0 5.16-.93 6.88-2.53l-3.35-2.6c-.93.62-2.12.99-3.53.99-2.7 0-5-1.83-5.82-4.28H2.74v2.69A10 10 0 0 0 12 22Z"/>
                  <path fill="#FBBC05" d="M6.18 13.58A6 6 0 0 1 5.86 12c0-.55.1-1.08.27-1.58V7.73H2.74A9.96 9.96 0 0 0 2 12c0 1.6.38 3.13 1.05 4.47l3.13-2.89Z"/>
                  <path fill="#EA4335" d="M12 6.16c1.52 0 2.89.53 3.96 1.55l2.97-2.97C17.16 3.13 14.8 2 12 2 8.06 2 4.65 4.27 2.74 7.73l3.4 2.69C7.02 7.99 9.31 6.16 12 6.16Z"/>
                </svg>
        ...
        <div className="row" style={{ justifyContent: 'space-between', fontSize: 12, color: 'var(--ink-4)' }}>
          <span>© 2026 Mar'my GED. Hébergement souverain · Saint-Denis, La Réunion.</span>
          <div className="row" style={{ gap: 14 }}>
            {['Confidentialité', 'CGU', 'Aide'].map((l) => (
              <a key={l} href="#" style={{ color: 'var(--ink-4)', textDecoration: 'none' }}>{l}</a>
            ))}
          </div>
        </div>
      </motion.section>
      ...
      <motion.section
        ...
        style={{ background: '#0b0f1a', color: '#fff', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '40px 60px', position: 'relative', overflow: 'hidden' }}
      >
        <div style={{
