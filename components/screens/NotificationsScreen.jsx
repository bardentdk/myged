...
const PrefItem = ({ label, icon, defaultOn }) => {
  const [on, setOn] = useState(defaultOn);
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 18px', borderBottom: '1px solid var(--line)' }}>
        ...
        <div>
          ...
          {shown.length === 0 ? (
            ...
            <div className="card" style={{ overflow: 'hidden' }}>
              {shown.map((n, i) => (
                <div key={n.id}
                  onClick={() => { markRead(n.id); if (n.docId) router.push(`/documents/${n.docId}`); }}
                  style={{
        ...
        {preferences && (
          <div className="card" style={{ height: 'fit-content', position: 'sticky', top: 76 }}>
            ...
            <div style={{ padding: '8px 0' }}>
              {PREF_ITEMS.map((p, i) => (
                <PrefItem key={i} label={p.label} icon={p.icon} defaultOn={p.defaultOn} />
              ))}
            </div>
