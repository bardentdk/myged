    ...
    <div className="page" style={{ paddingTop: 22 }}>
      ...
      <div className="card" style={{ padding: '12px 16px', marginBottom: 16, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ flex: 1, minWidth: 200, display: 'flex', alignItems: 'center', gap: 8, padding: '0 12px', background: 'var(--surface-2)', border: '1px solid var(--line)', borderRadius: 8, height: 34 }}>
          ...
          <input placeholder="Filtrer par document…" value={search} onChange={e => setSearch(e.target.value)}
              ...
              return (
                <tr key={a.id}>
                  ...
                  <td className="col-name" style={{ fontSize: 13 }}>
                    {a.doc
                      ? <span style={{ color: 'var(--accent)', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => router.push(`/documents/${a.doc}`)}>{a.what}</span>
