        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 10, padding: '0 14px', background: '#fff', border: '1.5px solid var(--accent)', borderRadius: 10, height: 48, boxShadow: '0 0 0 4px var(--accent-soft)' }}>
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)}
              placeholder="Nom, texte OCR, apprenant, session, tag…"
        ...
        <div className="card" style={{ padding: '14px 18px', marginBottom: 18 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr) auto', gap: 14, alignItems: 'end' }}>
            ...
            <div>
              ...
              <input className="input" placeholder="qualiopi, contrat…" value={filters.tags} onChange={(e) => setFilters((f) => ({ ...f, tags: e.target.value }))} />
            </div>
            <div className="col" style={{ gap: 8 }}>
              ...
              <button type="button" className="btn sm ghost" onClick={() => setFilters({ type: 'all', status: 'all', author: 'all', expiring: false, tags: '' })}>
                Réinitialiser
              </button>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10, marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--line)', flexWrap: 'wrap' }}>
            ...
            {['Jean Payet', 'convention OPCO', 'audit qualiopi 2026', 'emargement mars'].map((s) => (
              <button key={s} type="button" className="btn sm ghost" style={{ padding: '3px 8px', fontSize: 12 }} onClick={() => setQuery(s)}>
            ...
            <div className="row" style={{ marginBottom: 12, justifyContent: 'space-between' }}>
              ...
              <div className="row" style={{ gap: 6 }}>
                ...
                <select className="input" style={{ width: 'auto', height: 28, fontSize: 12, padding: '0 8px' }}>
                  <option>Pertinence</option><option>Date</option><option>Nom</option>
                </select>
              </div>
            </div>
                ...
                : results.map((doc, i) => (
                  <div key={doc.id} onClick={() => setPreview(doc)}
                    style={{ display: 'flex', gap: 14, padding: '14px 18px', borderBottom: i < results.length - 1 ? '1px solid var(--line)' : 0,
                      ...
                      borderLeft: preview?.id === doc.id ? '3px solid var(--accent)' : '3px solid transparent' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div className="micro row" style={{ gap: 6, marginBottom: 6 }}>
                        <Icon name="folder" size={11} /><span>{doc.folder}</span>
                        <span>·</span><span className="mono">v{doc.version}</span>
                      ...
                      <div style={{ fontSize: 12.5, color: 'var(--ink-3)', lineHeight: 1.5, background: 'var(--surface-2)', padding: '6px 10px', borderRadius: 6, borderLeft: '2px solid var(--line-strong)' }}>
                        …conformément aux dispositions relatives aux <mark style={{ background: '#fef08a', padding: '0 1px', borderRadius: 2 }}>conventions</mark> de formation passées avec l'<mark style={{ background: '#fef08a', padding: '0 1px', borderRadius: 2 }}>OPCO</mark> Akto…
                      </div>
                    ...
                    <div className="col" style={{ gap: 6, alignItems: 'flex-end', flexShrink: 0 }}>
                      ...
                      <button className="btn sm ghost" onClick={(e) => e.stopPropagation()}><Icon name="download" size={12} /></button>
                    </div>
                  </div>
                ))}
            </div>
            ...
            <div className="card" style={{ position: 'sticky', top: 76, height: 'fit-content', maxHeight: 'calc(100vh - 120px)', overflow: 'auto' }}>
              ...
              <div style={{ padding: '14px 18px' }}>
                ...
                <div className="row" style={{ gap: 8, marginTop: 14 }}>
                  <button className="btn primary sm" style={{ flex: 1, justifyContent: 'center' }} onClick={() => router.push(`/documents/${preview.id}`)}>Ouvrir le document</button>
